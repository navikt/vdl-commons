{{ config(materialized="table", cluster_by=["dato", "dim_nivaa"]) }}

with

    dim_tid as (select * from {{ ref("int_dim_tid__finn_neste_virkedag") }}),
    ...
    final as (
        ...
        from dim_tid
        left join last_year on dim_tid.pk_dim_tid = last_year.pk_dim_tid
        left join tertial on dim_tid.pk_dim_tid = tertial.pk_dim_tid
    )
